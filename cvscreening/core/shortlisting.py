...
google2 = ChatGoogleGenerativeAI(
    temperature=0,
    model="gemini-1.5-flash",
).with_fallbacks([google])
...
def get_data(file_path):
    ...
    if len(pages) > 1:
        ...
        return [Document(page_content=pdfstring, metadata=metadata)][0]
...
cv_score_cal = (
    template | llm_with_tools | PydanticToolsParser(tools=[cv_score]) | process_score
)
...
content_chain = (
    ...
    | RunnablePassthrough.assign(cv_score=(lambda x: x) | cv_score_cal)
)
