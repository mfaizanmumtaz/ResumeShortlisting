...
def data_validation(
    files: List[UploadFile], api_key: str, percentage: int, job_description: str
):
    ...
    if not (5 <= len(files) <= 900):
        raise HTTPException(
            status_code=400, detail="Number of files must be between 20 and 900."
        )
    ...
    if job_description_length < 20 or job_description_length > 1000:
        raise HTTPException(
            status_code=400, detail="Job description must be between 20 and 1000 words."
        )
